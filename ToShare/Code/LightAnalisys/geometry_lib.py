...
class circle:
	...
	def randomPoint(self, N):
		...
		maxNumber = int(np.floor(2*self.r + 4 * np.sum([np.sqrt(self.r**2 - i**2) for i in range(1, int(np.floor(self.r)))])))
	...
	def extimateCoefficients(self, image, N = 9, median = True):
		"""Extimate the rendering coefficients of the sphere using N points."""

		if median:
			ndimage.median_filter(self, size=9)
...
if __name__ == '__main__':
	...
	originalImage = np.asarray(Image.open("./../../Samples/DallE2/DallE2_2.png"), dtype=np.uint8)
